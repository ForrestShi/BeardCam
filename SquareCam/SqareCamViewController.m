...
@synthesize beardImage = _beardImage;
										  ...
										  frontFacing:(BOOL)isFrontFacing
{
    ...
    UIImage *rotatedSquareImage = [self.beardImage imageRotatedByDegrees:rotationDegrees];
...
- (void)drawFaceBoxesForFeatures:(NSArray *)features forVideoBox:(CGRect)clap orientation:(UIDeviceOrientation)orientation
{
	...
	for ( CIFaceFeature *ff in features ) {
        ...
        CGPoint mousePoint = [ff mouthPosition];
        ...
        NSLog(@"mouse position %@", NSStringFromCGPoint(mousePoint));
        NSLog(@"left eye %@", NSStringFromCGPoint([ff leftEyePosition]) );
        NSLog(@"right eye %@", NSStringFromCGPoint([ff rightEyePosition]) );

        NSLog(@"bounds %@", NSStringFromCGRect(self.view.layer.bounds));
		...
		if ( !featureLayer ) {
            ...
            [featureLayer setContents:(id)[self.beardImage CGImage]];
